...
export class StudentGrid {
    ...
    async addStudentIntoDb(student) {
        try {
            const addedStudents = await this.studentService.addStudent(student);
    ...
    catchEvents() {
        $('#btnAdd').click(() => {
            ...
            const student = {
                name: $(addRow.cells[0]).find('input').val(),
                gender: $(addRow.cells[1]).find('input').val(),
                country: $(addRow.cells[2]).find('input').val(),
                city: $(addRow.cells[3]).find('input').val()
            }
        ...
        $('body').on('click', 'table .btn-delete', async (el) => {
            var row = $(el.target).parents('tr');
            var studentId = Number(row.data('id'));

            try {
                const noOfRowsDeleted = await this.studentService.removeStudent(studentId);
        ...
        $('body').on('click', 'table .btn-edit', (el) => {
            var row = $(el.target).parents('tr');
        ...
        $('body').on('click', 'table .btn-cancel-update', async (el) => {
            var row = $(el.target).parents('tr');
            try {
                const students = await this.studentService.getStudentById(Number(row.data('id')));
        ...
        $('body').on('click', 'table .btn-update', async (el) => {
            var row = $(el.target).parents('tr');
            var student = {
            ...
            var studentId = Number(row.data('id'));
            try {
                const noOfRowsUpdated = await this.studentService.updateStudentById(studentId, student);
