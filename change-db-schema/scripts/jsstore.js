/*!
 * @license :jsstore - V4.8.2 - 01/11/2024
            ...
            _this.executeMiddleware_(request).then(function () {
                ...
                if (_this.requestQueue_.length === 0) {
                    ...
                    if (_this.isDbIdle_ && _this.isConOpened_) {
                        var isConnectionApi = [API.InitDb, API.CloseDb, API.DropDb, API.OpenDb, API.Terminate].indexOf(request.name) >= 0;
                        if (!isConnectionApi) {
                            _this.openDb_();
                        }
                    }
...
;// CONCATENATED MODULE: ./src/main/map.ts
